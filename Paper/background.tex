...
The luminosity can be used to find the mass, but only by using approximations for other values such as density, which can be estimated due to the structural analysis of meteorite collections. As luminosity is the only needed data point, a single all-sky camera is just as viable for measuring the population distribution. This process is described in detail in the methods section of this paper. 
